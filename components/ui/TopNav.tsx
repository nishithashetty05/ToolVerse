...
export default function TopNav({ onMenuClick }: { onMenuClick?: () => void }) {
  return (
    <header className="bg-white/80 dark:bg-[#0f172a]/80 backdrop-blur-md border-b border-gray-200 dark:border-gray-800 sticky top-0 z-30 h-16 flex items-center justify-between px-4 sm:px-6 lg:px-8">
      ...
      <div className="flex items-center gap-4">
        <div className="relative hidden sm:block">
          <input
            ...
            className="w-full sm:w-64 bg-gray-100 dark:bg-gray-800 border-none rounded-full pl-10 pr-4 py-2 text-sm focus:ring-2 focus:ring-primary focus:outline-none transition-all"
          />
          <Search className="absolute left-3 top-2.5 h-4 w-4 text-gray-400" />
        </div>

        <button className="relative p-2 text-gray-500 hover:text-primary rounded-full hover:bg-gray-100 dark:hover:bg-gray-800 transition-colors">
        ...
        >
          ...
          <span className="text-sm font-medium hidden sm:block dark:text-gray-200 text-gray-700">Student User</span>
        </Link>
      </div>
    </header>
  );
